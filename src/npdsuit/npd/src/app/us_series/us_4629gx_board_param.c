...
long us4629gx_board_npd_os_upgrade(unsigned int slot_index)
{
	npd_syslog_err("NH us4628pwr not be supported.\n");
	return NPD_SUCCESS;
}

ipp_fix_param_t  us4629gx_board_ipp_param =
{
    .ipp_portnum = 1,
...
long us4629gx_sub_slot_inserted(unsigned long sub_slot)
{
    int op_ret;
    music_subcard_info_t subcard_info;

    npd_syslog_dbg("us4629gx sub slot num is %d\n", sub_slot);
    subcard_info.index = sub_slot;
    op_ret = nbm_subcard_info_operate(&subcard_info);
    if (op_ret != 0)
    {
        npd_syslog_err("subcard isset info error!.\n");
        return -1;
    }
...
/*dynamic use*/
long us4629gx_sub_slot_insert_detect()
{
    return NPD_SUCCESS;
}

long us4629gx_sub_slot_board_type_get(unsigned long sub_slot, unsigned long *type)
{
     int op_ret;
    music_subcard_info_t subcard_info;

   npd_syslog_dbg("us4629gx board type sub slot num is %d\n", sub_slot);
    subcard_info.index = sub_slot;
    op_ret = nbm_subcard_info_operate(&subcard_info);
    if (op_ret != 0)
    {
        npd_syslog_err("subcard board type info error!.\n");
        return -1;
    }
	/*1 == XFP temp*/
...
long us4629gx_sub_slot_board_init(unsigned long sub_slot)
{
    
    return NPD_SUCCESS;
}

int us4629gx_sub_slot_man_param_get(unsigned int slot_index,   board_man_param_t *man_param)
{
	
    return NPD_SUCCESS;
}

sub_board_fix_param_t us4629gx_board_subboard_param = 
{
    .sub_slotnum = 3,     // more than 1
    .sub_slot_portnum = 5,
    .sub_slot_inserted = us4629gx_sub_slot_inserted,
    .sub_slot_insert_detect = us4629gx_sub_slot_insert_detect,
    .sub_slot_board_type_get = us4629gx_sub_slot_board_type_get,
    .sub_slot_board_init = us4629gx_sub_slot_board_init,
    .sub_slot_man_param_get  = us4629gx_sub_slot_man_param_get
};


temper_fix_param_t us4629gx_board_temper_param =
{
    .num = 0,
};

board_feature_t us4629gx_board_feature = 
{
    .jumbo_size = 16384,
    .max_macaddr = 16384,
    .max_macapp = 512,
...
int us4629gx_board_support_product[] =
{
    PRODUCT_US3000,
    0
};
...
board_fix_param_t us4629gx_board_param =
{
    .board_code = PPAL_HWCODE_US_4629GX,
    .board_type = PPAL_BOARD_TYPE_US_4629GX,
    .full_name = "AU4600-29X BOARD",
    .short_name = "AU4600-29X",
    .have_pp = TRUE,
    .master_flag = FALSE,
    .service_flag = TRUE,
    .panel_portnum = 24,
    .subboard_fix_param = &us4629gx_board_subboard_param,
    .ipp_fix_param = &us4629gx_board_ipp_param,
    .temper_fix_param = &us4629gx_board_temper_param,
    .os_upgrade = &us4629gx_board_npd_os_upgrade,
    .feature = &us4629gx_board_feature,
    .sdk_type = SDK_ATHEROS,
    .board_support_product = (int *)us4629gx_board_support_product,
