...
static int aus4628_pwr_console_fd = -1;

long us4628pwr_board_sfp_online(unsigned long panel_port)
{
    return NPD_SUCCESS;
}

long us4628pwr_board_sfp_detect_start()
{
    return NPD_SUCCESS;    
}

long us4628pwr_board_sfp_info_get(unsigned long panel_port, fiber_module_man_param_t *param)
{
    return NPD_SUCCESS;
}

long us4628pwr_board_slotno_get()
{
    int tipc_node_temp = tipc_get_own_node();
	return (tipc_node_temp&0x0F) - 1;
}

long us4628pwr_board_local_reset()
{
    npd_syslog_err("NH MCU local reset not be supported.\n");
    return NPD_SUCCESS;
}

long us4628pwr_board_reset_type()
{
    npd_syslog_err("NH MCU reset type get not be supported.\n");
    return NPD_SUCCESS;
}

long us4628pwr_board_led_lighting(unsigned long status)
{
    npd_syslog_err("NH MCU led lighting not be supported.\n");	
    return NPD_SUCCESS;
}

long us4628pwr_board_pne_mon_start()
{
    npd_syslog_err("NH MCU p&e monitor not be supported.\n");
    return NPD_SUCCESS;
}

/*long us4628pwr_board_mnparam_get(board_man_param_t *info)
{
    info->id = PPAL_HWCODE_US_4629GX_PWR;
    info->sn = "1001";
    return NPD_SUCCESS;
}*/
...
void us4628pwr_board_sal_config_init_defaults(void)
{
...
int us4628pwr_board_set_com_config(int fd, int baud_rate, 
		int data_bits, char parity, int stop_bits)
{
	...
	{
		npd_syslog_dbg("AU4600-29XP set_com_config tcsetattr failed.\n");
		return NPD_FAIL;	
	}
	return NPD_SUCCESS;
}
...
long us4628pwr_board_poe_init()
{
	...
	if ((aus4628_pwr_console_fd = open(filename, O_RDWR | O_NOCTTY | O_NDELAY)) < 0)
	{
		
		npd_syslog_dbg("AU4600-29XP init poe open console failed.\n");
		return NPD_FAIL;
	}

	if (fcntl(aus4628_pwr_console_fd, F_SETFL, 0) < 0) 
	{
		
		npd_syslog_dbg("AU4600-29XP init poe fcntrl F_SETFL failed.\n");
		return NPD_FAIL;
	}
	if ((ret = us4628pwr_board_set_com_config(aus4628_pwr_console_fd, 19200, 8, 'N', 1)) < 0)
	{
		
		npd_syslog_dbg("AU4600-29XP init poe set_com_config failed.\n");
		return NPD_FAIL;			
	}
...
long us4628pwr_board_poe_operate(void *in_data, void *out_data)
{
	...
	len = write(aus4628_pwr_console_fd, request_buf, buf_len);
	if (len < 0 || len != buf_len)
	{
		
		npd_syslog_dbg("AU4600-29XP init poe write console error.\n");
		return -1;		
	}
	...
	while (1)
	{
		...
		FD_ZERO(&rfds);
		FD_SET(aus4628_pwr_console_fd, &rfds);
		select(aus4628_pwr_console_fd + 1, &rfds, NULL, NULL,&time_wait);
		if(FD_ISSET(aus4628_pwr_console_fd, &rfds)){
			read_len = read(aus4628_pwr_console_fd, response_buf + (15 - remain_length), remain_length);
	...
	if (0 != remain_length){
		...
		npd_syslog_dbg("AU4600-29XP init poe read console error.\n");
		return -1;	
	}
	sum = 0;
	for (i = 0; i < 13; i++)
	...
	if (sum != ((response_buf[13]<<8)|response_buf[14]))
	{
		...
		npd_syslog_dbg("AU4600-29XP init poe read data checksum error.\n");
		return NPD_FAIL;
	}
...
long us4628pwr_board_local_conn_init(int product_type)
{

    return 0;  
}

long us4628pwr_board_system_conn_init(
    int product_type, 
...
poe_module_fix_param_t  us4628pwr_board_poe_param = 
{
	.poe_ports = {0xffffff,0},/*fixme?*/
	.pse_total_power = 3850,/*385W*/
	.pse_guard_band = 150,/*15W*/
	.pse_mpss = 1,
	.pse_type = PSE_IEEE_POEPLUS,
	.poe_init = &us4628pwr_board_poe_init,
	.poe_operate = &us4628pwr_board_poe_operate,
};

fiber_module_fix_param_t us4628pwr_board_sfp_param =
{
    .fiber_module_inserted = &us4628pwr_board_sfp_online,
    .fiber_module_insert_detect = &us4628pwr_board_sfp_detect_start,
    .fiber_module_info_get = &us4628pwr_board_sfp_info_get,
};
extern long nam_asic_info_get(int unit, struct ams_info_s *info);
extern long nam_asic_enable(int index);

long us4628pwr_board_asic_led_proc(int unit)
{
...
ams_fix_param_t us4628pwr_board_asic_switch =
{
    ...
    .ams_led_proc = &us4628pwr_board_asic_led_proc,
    .ams_enable = NULL,
};
ams_fix_param_t us4628pwr_board_ctrl_switch = 
{
...
board_spec_fix_param_t us4628pwr_board_spec_param =
{
    .board_type = PPAL_BOARD_TYPE_US_4629GX_PWR,
    .fiber_module_fix_param = &us4628pwr_board_sfp_param,
    .poe_module_fix_param = &us4628pwr_board_poe_param,
    .ams_param = {
                    [0] = &us4628pwr_board_asic_switch
                 },
    .slotno_get = &us4628pwr_board_slotno_get,
    .reset = &us4628pwr_board_local_reset,
    .get_reset_type = &us4628pwr_board_reset_type,
    .sys_led_lighting = &us4628pwr_board_led_lighting,
    .pne_monitor_start = &us4628pwr_board_pne_mon_start,
    .board_man_param_get = &us4628pwr_board_mnparam_get,
    .local_conn_init = &us4628pwr_board_local_conn_init,
    .system_conn_init = &us4628pwr_board_system_conn_init,
    .asic_config_init = &us4628pwr_board_sal_config_init_defaults
};
