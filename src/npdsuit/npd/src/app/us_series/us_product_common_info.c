...
unsigned long us_family_type_get()
{
	return nbm_get_family_type_get();
}

unsigned long us_product_hw_code_get()
{
	return nbm_get_product_hw_code();
}

unsigned long us_local_module_hw_code_get()
{
	return nbm_get_module_type();
	//return nbm_get_module_hw_code();
}

unsigned char us_board_hw_version_get()
{
	return nbm_get_board_hw_version();
}

unsigned char us_board_hw_slotno_get()
{
	return 0;
}

long us_load_backinfo(product_man_param_t *param)
{
...
void us_chassis_show()
{
    npd_syslog_err("chassis show now not be supported.\n"); 
}

long us_board_online(unsigned long slot_index)
{
	return nbm_board_online(slot_index);
}

long us_board_detect_start()
{
    return NPD_SUCCESS;
}

long us_master_board_online(unsigned long slot_index)
{
    return TRUE;
}

long us_board_reset(unsigned long slot_index)
{
	return nbm_board_reset(slot_index);
}

long us_board_poweroff(unsigned long slot_index)
{
    npd_syslog_err("board power off not be supported.\n");
    return NPD_SUCCESS;
}
int us_main_board_slot_support(int product_type, int slot, int subslot)
{
	if(subslot > 0)
		return FALSE;
	return TRUE;
...
long us_pne_monitor_start()
{
    npd_syslog_err("pne monitor not be supported.\n");
    return NPD_SUCCESS;
}

long us_fan_speed_adjust(unsigned long index, unsigned long speed)
{
	int ret = 0;
...
long us_os_upgrade(unsigned int slot_index)
{
	return chasm_os_upgrade(slot_index);
}
void us_power_monitor(void)
{
	int inserted = 0;
	int power_state = 0;
	int ret = 0;
...
void us_led_state_operate(void)
{
	int index = 0;
...
int us_pne_monitor(void)
{
	...
	do
	{
		...
		us_led_state_operate();
	}while(1);
}

long us_power_man_param_init(power_param_t * param)
{
...
long us_fan_param_init(fan_param_t * param)
{
...
int us_read_sysinfo_file(char * filename, char * buf, int len)
{
	int fd;
	int size = 0;
...
int us_board_mnparam_get(board_man_param_t *info)
{
	char mod_name[32];
	char mod_sn [32];
	char * ptr;
	...
	*ptr = '\0';
	...
	ret = us_read_sysinfo_file(MODULE_SN_FILE, mod_sn, 32);
	if (ret != NPD_SUCCESS)
	{
...
void us_int_power_handler(unsigned char int_value, unsigned char add_value)
{
