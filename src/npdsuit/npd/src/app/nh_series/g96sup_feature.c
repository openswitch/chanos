...
long g96sup_sfp_online(unsigned long panel_port)
{
    return NPD_SUCCESS;
}

long g96sup_sfp_detect_start()
{
    return NPD_SUCCESS;    
}

long g96sup_sfp_info_get(unsigned long panel_port, fiber_module_man_param_t *param)
{
    return NPD_SUCCESS;
}

long g96sup_slotno_get()
{
	return nbm_slotno_get();
}

long g96sup_local_reset()
{
	system("reboot");
    nbm_local_reset();
	return NPD_SUCCESS;
...
long g96sup_reset_type()
{
    npd_syslog_err("NH MCU reset type get not be supported.\n");
    return NPD_SUCCESS;
}

long g96sup_led_lighting(unsigned long status)
{
    npd_syslog_err("NH TGM9002 led lighting not be supported.\n");
    return NPD_SUCCESS;
}

void g96sup_mng_port_led_init()
{

}

long g96sup_asic_led_proc(int unit)
{
    return NPD_SUCCESS;
}

long g96sup_pne_mon_start()
{
    npd_syslog_err("NH G96SUP p&e monitor not be supported.\n");
    return NPD_SUCCESS;
}

long g96sup_mnparam_get(board_man_param_t *info)
{
	npd_syslog_dbg("G96SUP man param get.\n");
	
    info->id = PPAL_BOARD_HWCODE_TSM9002_LOONGSON;
...
long g96sup_sal_config_init_defaults(void)
{
...
static bcm_pbmp_t g96sup_internal_pbmp[MAX_ASIC_NUM_PER_BOARD];
static bcm_pbmp_t g96sup_fabric2fabric_pbmp[MAX_ASIC_NUM_PER_BOARD];
static bcm_pbmp_t g96sup_fabric2line_pbmp[MAX_ASIC_NUM_PER_BOARD];
static bcm_pbmp_t g96sup_panel_pbmp[MAX_ASIC_NUM_PER_BOARD];
static bcm_pbmp_t g96sup_fabric2extserv_pbmp[MAX_ASIC_NUM_PER_BOARD];

static int g96sup_pbmp_has_set[MAX_ASIC_NUM_PER_BOARD] = {0};

long g96sup_local_pbmp_get(int unit, bcm_pbmp_t* int_pbmp, 
	bcm_pbmp_t* f2f_pbmp, bcm_pbmp_t* f2l_pbmp, bcm_pbmp_t* panel_pbmp)
{
	if (0 == g96sup_pbmp_has_set[unit])
	{
		...
		BCM_PBMP_ASSIGN(g96sup_internal_pbmp[unit], internal_pbmp);
		BCM_PBMP_ASSIGN(g96sup_fabric2fabric_pbmp[unit], fabric2fabric);
		BCM_PBMP_ASSIGN(g96sup_fabric2line_pbmp[unit], fabric2line);
		BCM_PBMP_ASSIGN(g96sup_panel_pbmp[unit], panel);			
		g96sup_pbmp_has_set[unit] = 1;
	}

	BCM_PBMP_ASSIGN(*int_pbmp, g96sup_internal_pbmp[unit]);
	BCM_PBMP_ASSIGN(*f2f_pbmp, g96sup_fabric2fabric_pbmp[unit]);
	BCM_PBMP_ASSIGN(*f2l_pbmp, g96sup_fabric2line_pbmp[unit]);
	BCM_PBMP_ASSIGN(*panel_pbmp, g96sup_panel_pbmp[unit]);			

	return 0;
}

long g96sup_local_conn_standby()
{
...
long g96sup_get_stk_trunk_by_peer_slotunit(unsigned char unit, 
...
long g96sup_local_conn_init(int product_type)
{
    int unit;
    int port;
    int rv;
...
long g96sup_system_sdkdiff_conn_init(
    int product_type, 
    ...
    )
{
    ...
    for(unit = 0; unit < nam_asic_get_instance_num(); unit++)
    {
		...
		g96sup_local_pbmp_get(unit, &internal_pbmp, 
...
long g96sup_system_sdkdiff_conn_deinit(
    int product_type, 
    int del_board_type, 
    int del_slotid    
    )
...
long g96sup_system_indpnt_conn_init(
    int product_type, 
    int insert_board_type, 
    int insert_slotid    
    )
...
long g96sup_system_indpnt_conn_deinit(
    int product_type, 
    int del_board_type, 
    int del_slotid    
    )
...
long g96sup_system_extserv_conn_init(
    int product_type, 
    int insert_board_type, 
    int insert_slotid    
    )
{
    ...
    for(unit = 0; unit < nam_asic_get_instance_num(); unit++)
    {
		...
		BCM_PBMP_OR(g96sup_fabric2extserv_pbmp[unit], conn_pbmp);		

	}

    for(unit = 0; unit < nam_asic_get_instance_num(); unit++)
	{
		/* PHASE 6 board Special Configure */
	}
...
long g96sup_system_extserv_conn_deinit(
    int product_type, 
    int del_board_type, 
    int del_slotid    
    )
{
    ...
    for(unit = 0; unit < nam_asic_get_instance_num(); unit++)
    {
		...
		BCM_PBMP_REMOVE(g96sup_fabric2extserv_pbmp[unit], conn_pbmp);				
    }		
	return 0;
}

long g96sup_system_fabric_conn_init(
    int product_type, 
    int insert_board_type, 
    int insert_slotid    
    )
...
long g96sup_system_fabric_conn_deinit(
    int product_type, 
    int del_board_type, 
    int del_slotid    
    )
{
    ...
    for(unit = 0; unit < nam_asic_get_instance_num(); unit++)
    {
		...
		g96sup_local_pbmp_get(unit, &internal_pbmp, 
					&fabric2fabric, &fabric2line, &panel);
		BCM_PBMP_ASSIGN(fabric2extserv, g96sup_fabric2extserv_pbmp[unit]);
    ...
    )
{
	...
	{
		return g96sup_system_sdkdiff_conn_init(product_type, 
			insert_board_type, insert_slotid);
	}
	if(SYS_MODULE_SLOT_EXTERNAL_SERVICE(insert_slotid))
	{
		return g96sup_system_extserv_conn_init(product_type,
			insert_board_type, insert_slotid);
	}
	if(SYS_MODULE_SLOT_INDEPENDENT_SERVICE(insert_slotid))
	{
		return g96sup_system_indpnt_conn_init(product_type, 
			insert_board_type, insert_slotid);
	}
	if(SYS_CHASSIS_SLOTNO_ISFABRIC(insert_slotid))
	{
		return g96sup_system_fabric_conn_init(product_type,
			insert_board_type, insert_slotid);
	}	
    ...
    for(unit = 0; unit < nam_asic_get_instance_num(); unit++)
    {
		...
		BCM_PBMP_ITER(conn_pbmp, port)
		{
			...
			rv = g96sup_get_stk_trunk_by_peer_slotunit(
...
long g96sup_system_conn_deinit(
    int product_type, 
    int del_board_type, 
    int del_slotid    
    )
{
	...
	{
		return g96sup_system_sdkdiff_conn_deinit(product_type, 
			del_board_type, del_slotid);
	}
	if(SYS_MODULE_SLOT_EXTERNAL_SERVICE(del_slotid))
	{
		return g96sup_system_extserv_conn_deinit(product_type, 
			del_board_type, del_slotid);
	}
	if(SYS_MODULE_SLOT_INDEPENDENT_SERVICE(del_slotid))
	{
		return g96sup_system_indpnt_conn_deinit(product_type, 
			del_board_type, del_slotid);
	}
	if(SYS_CHASSIS_SLOTNO_ISFABRIC(del_slotid))
	{
		return g96sup_system_fabric_conn_deinit(product_type,
			del_board_type, del_slotid);
	}	
...
fiber_module_fix_param_t g96sup_sfp_param =
{
    .fiber_module_inserted = &g96sup_sfp_online,
    .fiber_module_insert_detect = &g96sup_sfp_detect_start,
    .fiber_module_info_get = &g96sup_sfp_info_get,
};
extern long nam_asic_info_get(int unit, struct ams_info_s *info);
extern long nam_asic_enable(int index);

ams_fix_param_t g96sup_asic_switch =
{
    ...
    .ams_led_proc = &g96sup_asic_led_proc,
};
ams_fix_param_t g96sup_ctrl_switch = 
{
...
board_spec_fix_param_t g96sup_spec_param =
{
    .board_type = PPAL_BOARD_TYPE_G96SUP,
    .fiber_module_fix_param = &g96sup_sfp_param,
    .ams_param = {
                    [0] = &g96sup_asic_switch,
                    [2] = &g96sup_ctrl_switch
                 },
    .slotno_get = &g96sup_slotno_get,
    .reset = &g96sup_local_reset,
    .get_reset_type = &g96sup_reset_type,
    .sys_led_lighting = &g96sup_led_lighting,
    .pne_monitor_start = &g96sup_pne_mon_start,
    .board_man_param_get = &t9000_board_mnparam_get,
    .local_conn_init = &g96sup_local_conn_init,
    .system_conn_init = &g96sup_system_conn_init,
    .system_conn_deinit = &g96sup_system_conn_deinit,
    .asic_config_init = &g96sup_sal_config_init_defaults
};
