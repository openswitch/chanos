...
long g9604x_sfp_online(unsigned long panel_port)
{
    return NPD_SUCCESS;
}

long g9604x_sfp_detect_start()
{
    return NPD_SUCCESS;    
}

long g9604x_sfp_info_get(unsigned long panel_port, fiber_module_man_param_t *param)
{
    return NPD_SUCCESS;
}

long g9604x_slotno_get()
{
	return nbm_slotno_get();
}

long g9604x_local_reset()
{
	return nbm_local_reset();
}

long g9604x_reset_type()
{
    npd_syslog_err("NH G9604X reset type get not be supported.\n");
    return NPD_SUCCESS;
}

long g9604x_led_lighting(unsigned long status)
{
    npd_syslog_err("NH G9604X led lighting not be supported.\n");
    return NPD_SUCCESS;
}

long g9604x_asic_led_proc(int unit)
{
    npd_syslog_dbg("NH G9604X led code loading.\n");
    int i = 0;

    for(i = 0; i < 4; i++)
    {
        xgphy_reg_write(i, 1, 0xd006, 9);
        xgphy_reg_write(i, 1, 0xd007, 0x12);
        xgphy_reg_write(i, 1, 0xd008, 0x1a);
        xgphy_reg_write(i, 4, 0xc210, 0x1b1b);
        xgphy_reg_write(i, 1, 0xc301, 0x2084);
    }
	return 0;
}

long g9604x_pne_mon_start()
{
    npd_syslog_err("NH G9604X p&e monitor not be supported.\n");
    return NPD_SUCCESS;
}

long g9604x_mnparam_get(board_man_param_t *info)
{
    info->id = PPAL_BOARD_HWCODE_G9604X;
	strncpy(info->sn, "1000", strlen("1000"));

	return t9000_board_mnparam_get(info);
}
extern void lcm_gchip_id_preconfig(int gchip_id);
extern void lcm_sdk_board_type_set(int board_type);
void g9604x_sal_config_init_defaults(void)
{
    lcm_gchip_id_preconfig(UNIT_2_MODULE(SYS_LOCAL_MODULE_TYPE,
                                         SYS_LOCAL_MODULE_SLOT_INDEX, 0, 0));
	lcm_sdk_board_type_set(1);
}


long g9604x_local_conn_init(int product_type)
{
	int ret = 0;
	int i = 0;
...
long g9604x_system_conn_deinit(
    int product_type, 
    int del_board_type, 
    int del_slotid    
    )
...
long g9604x_system_master_switch(
    int product_type, 
...
fiber_module_fix_param_t g9604x_sfp_param =
{
    .fiber_module_inserted = &g9604x_sfp_online,
    .fiber_module_insert_detect = &g9604x_sfp_detect_start,
    .fiber_module_info_get = &g9604x_sfp_info_get,
};
long g9604x_nam_asic_enable()
{
	int ret = 0;
	int i = 0;
	unsigned char unit_no = 0;
...
ams_fix_param_t g9604x_asic_switch =
{
    ...
    .ams_led_proc = &g9604x_asic_led_proc,
};

board_spec_fix_param_t g9604x_spec_param =
{
    .board_type = PPAL_BOARD_TYPE_G9604X,

    .fiber_module_fix_param = &g9604x_sfp_param,
    .ams_param = {
                    [ASIC_SWITCH_TYPE] = &g9604x_asic_switch,
                 },
    .slotno_get = &g9604x_slotno_get,
    .reset = &g9604x_local_reset,
    .get_reset_type = &g9604x_reset_type,
    .sys_led_lighting = &g9604x_led_lighting,
    .pne_monitor_start = &g9604x_pne_mon_start,
    .board_man_param_get = &t9000_board_mnparam_get,
    .local_conn_init = &g9604x_local_conn_init,
    .system_conn_init = &g9604x_system_conn_init,
    .system_conn_deinit = &g9604x_system_conn_deinit,
    .asic_config_init = &g9604x_sal_config_init_defaults,
    .asic_after_data_sync_conf = &g9604x_nam_asic_enable,
    .master_switch = &g9604x_system_master_switch
};
