...
void t9000_led_state_operate(void)
{
	int index = 0;
	...
	if (SYS_LOCAL_MODULE_WORKMODE_ISMASTER)
	{
		...
		if (SYS_LOCAL_MODULE_RUNNINGSTATE == LOCAL_ACTMASTER_RUNNING
			|| SYS_LOCAL_MODULE_RUNNINGSTATE == LOCAL_SBYMASTER_RUNNING)
		{
			led_op.state = LED_STATUS_NORMAL;
		}
		else
	...
	/* Added for MASTERSTANDBY in AS9603 */
	if (SYS_LOCAL_MODULE_ISMASTERSTANDBY && 
			(FULL_MESH == SYS_PRODUCT_TOPO))
	{
		memset(&led_op, 0, sizeof(led_op_args));
		led_op.inserted = LED_STATUS_INSERT;
		if (SYS_LOCAL_MODULE_RUNNINGSTATE == LOCAL_SBYMASTER_RUNNING)
		{
			led_op.state = LED_STATUS_NORMAL;
		}
		else
...
int t9000_pne_monitor(void)
{
	...
	do
	{
		...
		t9000_led_state_operate();
	}while(1);
}

long t9000_pne_monitor_start(void)
{
	 nam_thread_create("PneMonitorThread",(void*)t9000_pne_monitor, NULL,NPD_FALSE,NPD_FALSE);
	return NPD_SUCCESS;
}

long t9000_power_man_param_init(power_param_t * param)
{
...
long t9000_fan_param_init(fan_param_t * param)
{
...
int t9000_read_sysinfo_file(char * filename, char * buf, int len)
{
	int fd;
	int size = 0;
...
long t9000_board_mnparam_get(board_man_param_t *info)
{
	char mod_name[32];
	char mod_sn [32];
	char * ptr;
	...
	ret = t9000_read_sysinfo_file(OEM_MODULE_NAME_FILE,mod_name, 32);
	if (ret != NPD_SUCCESS)
	{
		npd_syslog_err("get oem module name error.\n");
		ret = t9000_read_sysinfo_file(MODULE_NAME_FILE,mod_name, 32);
		if (ret != NPD_SUCCESS)
		{
	...
	ret = t9000_read_sysinfo_file(MODULE_SN_FILE, mod_sn, 32);
	if (ret != NPD_SUCCESS)
	{
...
void t9000_npd_master_set(unsigned long value)
{
	nbm_master_set(value);
}

void t9000_int_power_handler(unsigned char int_value, unsigned char add_value)
{
	...
	npd_syslog_dbg("Enter t9000_int_power_handler.\n");
