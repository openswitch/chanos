...
unsigned long ds_family_type_get()
{
	return nbm_get_family_type_get();
}

unsigned long ds_product_hw_code_get()
{
	return nbm_get_product_hw_code();
}

unsigned long ds_local_module_hw_code_get()
{
	return nbm_get_module_type();
	//return nbm_get_module_hw_code();
}

unsigned char ds_board_hw_version_get()
{
	return nbm_get_board_hw_version();
}

unsigned char ds_board_hw_slotno_get()
{
	return 0;
}

long ds_load_backinfo(product_man_param_t *param)
{
...
void ds_chassis_show()
{
    npd_syslog_err("chassis show now not be supported.\n"); 
}

long ds_board_online(unsigned long slot_index)
{
	return nbm_board_online(slot_index);
}

long ds_board_detect_start()
{
    return NPD_SUCCESS;
}

long ds_master_board_online(unsigned long slot_index)
{
    return TRUE;
}

long ds_board_reset(unsigned long slot_index)
{
	return nbm_board_reset(slot_index);
}

long ds_board_poweroff(unsigned long slot_index)
{
    npd_syslog_err("board power off not be supported.\n");
    return NPD_SUCCESS;
}
int ds_main_board_slot_support(int product_type, int slot, int subslot)
{
	if(subslot > 0)
		return FALSE;
	return TRUE;
...
long ds_pne_monitor_start()
{
    npd_syslog_err("pne monitor not be supported.\n");
    return NPD_SUCCESS;
}

long ds_fan_speed_adjust(unsigned long index, unsigned long speed)
{
	int ret = 0;
...
void ds_led_state_operate(void)
{
	int index = 0;
...
int ds_pne_monitor(void)
{
	...
	do
	{
		...
		ds_led_state_operate();
	}while(1);
}

long ds_power_man_param_init(power_param_t * param)
{
...
long ds_fan_param_init(fan_param_t * param)
{
...
int ds_read_sysinfo_file(char * filename, char * buf, int len)
{
	int fd;
	int size = 0;
...
int ds_board_mnparam_get(board_man_param_t *info)
{
	char mod_name[32];
	char mod_sn [32];
	char * ptr;
	...
	ret = ds_read_sysinfo_file(MODULE_NAME_FILE,mod_name, 32);
	if (ret != NPD_SUCCESS)
	{
	...
	*ptr = '\0';
	...
	ret = ds_read_sysinfo_file(MODULE_SN_FILE, mod_sn, 32);
	if (ret != NPD_SUCCESS)
	{
...
void ds_int_power_handler(unsigned char int_value, unsigned char add_value)
{
	...
	npd_syslog_dbg("Enter us_int_power_handler.\n");
