...
//#define OEMINFO_ENCRYPT
#define ENCRYPT_PRINT_INFO 0
#define DECRYPT_PRINT_INFO 0
#define PRINT_FILE_CONTENT 0

/*
	u8 k[24] = {0x12, 0xed, 0x43, 0xbc, 0x56, 0xa9, 0x87, 0x78,
		0xcb, 0x34, 0xde, 0x21, 0x87, 0x78, 0x9a, 0x65, 
		0x25, 0xda, 0x69, 0x96, 0xa6, 0x59, 0x70, 0x8f};
*/
...
static int des_setkey(const u8 *key,  unsigned int keylen)
{
	u32 *flags = &crt_flags;
	...
	if ((ret == 0) && (*flags & CRYPTO_TFM_REQ_WEAK_KEY)) {
...
static void des_encrypt(u8 *dst, const u8 *src)
{
...
static void des_decrypt(u8 *dst, const u8 *src)
{
 ...
 *
 */
static int des3_ede_setkey(const u8 *key, unsigned int keylen)
{
	...
	if (!((K[0] ^ K[2]) | (K[1] ^ K[3])) ||
		...
		*flags |= CRYPTO_TFM_RES_WEAK_KEY;
		printf("des3_ede_setkey : ========== EINVAL\n");
		return -EINVAL;
	}

	des_ekey(expkey, key); 
	expkey += DES_EXPKEY_WORDS; 
	key += DES_KEY_SIZE;
	dkey(expkey, key); 
	expkey += DES_EXPKEY_WORDS; 
	key += DES_KEY_SIZE;
...
static void des3_ede_encrypt(u8 *dst, const u8 *src)
{
	...
	u32 L, R, A, B;
	int i;
#if ENCRYPT_PRINT_INFO
	static int en_print_flag = 0;
#endif
	L = le32_to_cpu(s[0]);
	R = le32_to_cpu(s[1]);
...
static void des3_ede_decrypt(u8 *dst, const u8 *src)
{
	...
	u32 L, R, A, B;
	int i;
#if DECRYPT_PRINT_INFO
	static int de_print_flag = 0;
#endif
	L = le32_to_cpu(s[0]);
	R = le32_to_cpu(s[1]);
...
int main(int argc, char *argv[])
{
	...
	int file_end = 0;
	if(argc != 3){
		printf("Please use %s file1 file2\n", argv[0]);
		return 1;
	}
/*
#if __BYTE_ORDER == __LITTLE_ENDIAN
	printf("The byte order of this host is little endian\n");
#elif __BYTE_ORDER == __BIG_ENDIAN
	printf("The byte order of this host is big endian\n");
#else
	printf("The byte order of this host is unknown\n");
#endif
*/
	//printf("Byte order test : 0x%08x\n", __swab32(0x12345678));
	...
	FILE *fp_rd = fopen(src_file, "rb");
	if(fp_rd == NULL){
		printf("fp_rd open error\n");
		return -1;
	}
	
	FILE *fp_wr = fopen(dst_file, "w+b");
	if(fp_wr == NULL){
		printf("fp_wr open error\n");
		fclose(fp_rd);
		return -1;
	}
	
	des3_ede_setkey(k, 24);

	while(!feof(fp_rd)){
		memset(data, 0, 8);
		ret = fread(data, 1, 8, fp_rd);
		if(ret < 8){
			file_end = 1;	
		}
...
#ifdef OEMINFO_ENCRYPT	
		des3_ede_encrypt(crypt, data);
#else
		des3_ede_decrypt(crypt, data);
#endif
		...
		if(print_cnt == 0){
			printf("WR : ");
			for(i = 0; i < 8; i++)
				printf("0x%02x ", crypt[i]);
			printf("\n");
		}
#endif
		ret = fwrite(crypt, 1, 8, fp_wr);
		if(ret < 8 || file_end == 1){
			break;
		}
#if PRINT_FILE_CONTENT		
